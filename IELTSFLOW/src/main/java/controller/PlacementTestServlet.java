...
/**
 * PlacementTestServlet — Handles Placement Test (Thi đầu vào):
 *   GET  /placement-test              → Intro page
 *   GET  /placement-test?action=start → Create submission, load questions, redirect to take
 *   GET  /placement-test?action=take  → Full-screen exam UI
 *   POST /placement-test?action=submit → Grade, save, redirect to result
 *   POST /placement-test?action=violation → Anti-cheat AJAX
 *   GET  /placement-test?action=result&submissionId=X → Show band + level recommendation
 */
@WebServlet(name = "PlacementTestServlet", urlPatterns = {"/placement-test"})
public class PlacementTestServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            switch (action) {
                case "take":   handleTake(req, resp);   break;
                case "result": handleResult(req, resp); break;
                default:       handleIndex(req, resp);  break;
            }
        } catch (Exception e) {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            switch (action) {
                ...
                default: resp.sendRedirect(req.getContextPath() + "/placement-test"); break;
            }
        } catch (Exception e) {
            req.setAttribute("errorMsg", "Đã xảy ra lỗi: " + e.getMessage());
            req.getRequestDispatcher("/jsp/error.jsp").forward(req, resp);
    ...
    private void handleIndex(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        Exam exam = examService.getRandomPlacementTest();
    ...
    private void handleStart(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        Exam exam = examService.getRandomPlacementTest();
        if (exam == null) {
            req.setAttribute("errorMsg", "Hiện tại chưa có đề thi đầu vào. Vui lòng thử lại sau.");
            req.getRequestDispatcher("/jsp/placement-test/index.jsp").forward(req, resp);
            return;
        }
        ...
        session.setAttribute("pt_currentExam", exam);
        session.setAttribute("pt_currentQuestions", questions);
        session.setAttribute("pt_currentSubmissionId", submissionId);
        session.setAttribute("pt_examStartTime", System.currentTimeMillis());
        resp.sendRedirect(req.getContextPath() + "/placement-test?action=take");
    }

    private void handleTake(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        HttpSession session = req.getSession(false);
        Exam exam = (Exam) session.getAttribute("pt_currentExam");
        if (exam == null) { resp.sendRedirect(req.getContextPath() + "/placement-test"); return; }
        req.setAttribute("exam", exam);
        req.setAttribute("questions", session.getAttribute("pt_currentQuestions"));
        req.setAttribute("submissionId", session.getAttribute("pt_currentSubmissionId"));
        req.setAttribute("maxViolations", MAX_VIOLATIONS);
        req.setAttribute("examMode", "placement");
    ...
    private void handleSubmit(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        HttpSession session = req.getSession(false);
        int submissionId = (Integer) session.getAttribute("pt_currentSubmissionId");
        List<Question> questions = (List<Question>) session.getAttribute("pt_currentQuestions");
        ...
        Object vc = session.getAttribute("violationCount_pt_" + submissionId);
        if (vc != null) violationCount = (int) vc;
        boolean forcedSubmit = Boolean.TRUE.equals(session.getAttribute("forcedSubmit_pt_" + submissionId));
        ...
        session.removeAttribute("pt_currentExam");
        session.removeAttribute("pt_currentQuestions");
        session.removeAttribute("pt_currentSubmissionId");
        session.removeAttribute("pt_examStartTime");
        session.removeAttribute("violationCount_pt_" + submissionId);
        session.removeAttribute("forcedSubmit_pt_" + submissionId);

        resp.sendRedirect(req.getContextPath() + "/placement-test?action=result&submissionId=" + submissionId);
    }

    private void handleViolation(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        HttpSession session = req.getSession(false);
        Object subIdObj = session != null ? session.getAttribute("pt_currentSubmissionId") : null;
        ...
        if (violations >= MAX_VIOLATIONS && session != null)
            session.setAttribute("forcedSubmit_pt_" + submissionId, true);
    ...
    private void handleResult(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        catch (NumberFormatException e) { resp.sendRedirect(req.getContextPath() + "/placement-test"); return; }
        int userId = (int) req.getSession().getAttribute("userId");
        TestSubmission sub = submissionService.getSubmissionById(subId);
        if (sub == null || sub.getUserId() != userId) {
            resp.sendRedirect(req.getContextPath() + "/placement-test"); return;
        }
        // Compute IELTS level label
        req.setAttribute("submission", sub);
        req.setAttribute("levelLabel", computeLevelLabel(sub.getOverallBand()));
        req.getRequestDispatcher("/jsp/placement-test/result.jsp").forward(req, resp);
    }

    /** Maps overall band to a beginner/intermediate/advanced label. */
    private String computeLevelLabel(Double band) {
        if (band == null) return "Chưa xác định";
        if (band < 4.0)  return "Beginner (Mới bắt đầu)";
        if (band < 5.5)  return "Elementary (Cơ bản)";
        if (band < 6.5)  return "Intermediate (Trung cấp)";
        if (band < 7.5)  return "Upper-Intermediate (Khá)";
